...
import { checkPhone, checkPassword, checkCode } from "@/unitls/validation.ts";
export default defineComponent({
  ...
  setup(props) {
    ...
    //自定义手机号校验
    const validatePhone = async (rule: any, value: string): Promise<void> => {
      if (value === "") {
    ...
    //自定义验证码校验
    const validatecode = async (rule: any, value: string): Promise<void> => {
      if (value === "") {
    ...
    //按钮信息
    const buttonInformation:button = reactive({
      name: "获取验证码",
      disable: false,
      loading: false,
      countdown: 60,
